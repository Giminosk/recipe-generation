...
from eat_pim.eatpim.etl.recipe_graph import RecipeGraph
...
class SemSimCalculator:
    ...
    def sim(self, a, b):
        ...
        common_preds = [
            p for p in common_preds if nx.has_path(self.G, self.root_node, p)
        ]
        ...
        for p in common_preds:
            ...
            if depth > max_depth:
                ...
                if p_ic > max_ic:
                    max_ic = p_ic
                    lca_node = p
        lca_pathdist = nx.shortest_path_length(
            self.G, lca_node, a
        ) + nx.shortest_path_length(self.G, lca_node, b)
...
def visualize(
    G, ingredient_nodes, external_ingredient_nodes, foodon_nodes, equipment_nodes
):
    ...
    nx.draw_networkx_nodes(
        G, pos, nodelist=list(external_ingredient_nodes), node_color="white"
    )
...
def process_single_recipe(
    ...
    while True:
        ...
        objset = set(
            obj
            for step in recipe_data["parsed_steps"].values()
            ...
            if obj in ing_checks:
                ...
                for ing in ing_checks:
                    if entity_subclass_dig.has_node(ing) and (
                        nx.has_path(entity_subclass_dig, obj, ing)
                        or nx.has_path(entity_subclass_dig, ing, obj)
                    ):
...
def convert_recipe_data_to_kg(*, data, cleanup_links, ing_list, processes):
    ...
    for wiki_file in ["cul_equip.nq", "food_prep.nq"]:
        ...
        wiki_kg.parse(
            str((path.DATA_DIR / f"wikidata_cooking/{wiki_file}").resolve()),
            format="nquads",
        )
        for pred in [
            rdflib.URIRef("http://www.wikidata.org/prop/direct/P31"),  # instance of
    ...
    aps_dict = m.dict()
    sem_dist_calc = SemSimCalculator(
        G=reversed_entity_subclass_dig, shared_ic_dict=aps_dict
    )

    pool = mp.Pool(
        processes,
        initializer=process_single_recipe,
        initargs=(
            q,
            recipe_to_tree,
            entity_subclass_dig,
    ...
    for recipe_id in recipe_ids:
        ...
        if progress % 1000 == 0:
            print(
                f"progress: {round(progress/total_items, 3)}, time: {time.time()-starttime}"
            )
        # if progress == 20:#00:
        #     break
...
def main(
    *,
    ...
    (
        ...
        entity_subclass_dig,
        obj_connection_info,
    ) = convert_recipe_data_to_kg(
        data=data, ing_list=ing_list, cleanup_links=parsed_links, processes=processes
    )
