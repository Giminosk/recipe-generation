...
def match_objects_and_ings(
    *,
    ...
    for noun in noun_names:
        ...
        if checked % 1000 == 0:
            print(
                f"progress: {round(checked / len(noun_names), 5)}, {round(time.time() - starttime, 5)}s"
            )
        ...
        if len(ing_match_candidates):
            ingmatch_sorted_by_len = sorted(
                ing_match_candidates.items(), key=lambda kv: kv[1], reverse=True
            )
            ing_matches = []
            ...
            while matches_updated:
                ...
                for ingstr, inglen in ingmatch_sorted_by_len:
                    ...
                    if ingstr_index != -1 and ingstr_index + inglen == len(
                        stripped_noun
                    ):
        ...
        if len(stripped_noun) <= 1 or (
            stripped_noun[0].isdigit() and stripped_noun[-1].isdigit()
        ):
            continue
        ...
        if len(subnoun_match):
            nounmatch_sorted_by_len = sorted(
                subnoun_match.items(), key=lambda kv: kv[1], reverse=True
            )
            noun_matches = []
            matches_updated = True
            while matches_updated:
                ...
                for noun_2, nlen in nounmatch_sorted_by_len:
                    ...
                    if noun_2_index != -1 and noun_2_index + nlen == len(stripped_noun):
                        ...
                        if (
                            ...
                            and noun_2_index > 1
                            and (
                                stripped_noun[noun_2_index - 2].isdigit()
                                or stripped_noun[noun_2_index - 1].isdigit()
                            )
                        ):
...
def match_ings_and_ings(*, ing_names, ing_name_CV, ing_name_vector):
    ...
    for ing in ing_names:
        ...
        if checked % 1000 == 0:
            print(
                f"progress: {round(checked / len(ing_names), 5)}, {round(time.time() - starttime, 5)}s"
            )
        ...
        if len(subing_match):
            stripped_noun = ing
            ingmatch_sorted_by_len = sorted(
                subing_match.items(), key=lambda kv: kv[1], reverse=True
            )
            ing_matches = []
            matches_updated = True
            while matches_updated:
...
def match_names_to_external(*, name_list, matcher: Matcher):
    ...
    for name in name_list:
        ...
        if checked % 1000 == 0:
            print(
                f"progress: {round(checked / max_count, 5)}, {round(time.time() - starttime, 5)}s"
            )
    print(f"names with no match: {no_foodon_match}")
    return links


def main(*, input_file, wiki_dir, foodon_dir, output_dir):
    ...
    inglist = list(
        set(ingr for recipe in recipe_data.values() for ingr in recipe["ingredients"])
    )
    ...
    obj_to_ing, obj_to_subobj, obj_leftovers = match_objects_and_ings(
        noun_names=objlist,
        noun_name_CV=noun_name_CV,
        noun_name_vector=noun_name_vector,
        ing_names=inglist,
        ing_name_CV=ing_name_CV,
        ing_name_vector=ing_name_vector,
    )

    ing_to_ing = match_ings_and_ings(
    ...
    obj_to_equipment = match_names_to_external(
        name_list=objlist, matcher=wiki_e_matcher
    )
    ...
    verb_to_preparations = match_names_to_external(
        name_list=verblist, matcher=wiki_p_matcher
    )
