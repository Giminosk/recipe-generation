...
from eat_pim.eatpim.utils import path
import numpy as np
import argparse
...
def main(
    *, input_file: Path, ingredient_file: Path, output_dir: Path, cleanup_file: Path
):
    ...
    for recipe, data in recipe_data.items():
        ...
        def get_pred_items(G, n):
            ...
            for _, out_node in recG.out_edges(n):
                if out_node[:4] == "pred":
                    output.append(get_pred_items(G, n=out_node))
                else:
                    output.append(out_node)
                    entity_names.add(out_node)
                    node_set.add(out_node)
                    formatted_triple_data.append(
                        (str(out_node), str(rdflib.RDF.type), str("Ingredient"))
                    )
            return {pred_name: output}

        pred_data = get_pred_items(recG, data["output_node"])
        # empty graph - possibly due to removing non-ingredient nodes.
        # in this case, just move on
        if not pred_data["RECIPE OUTPUT"]:
            continue

        formatted_graph_data.append(
            {data["output_node"]: pred_data["RECIPE OUTPUT"][0]}
        )

        for node in node_set:
            formatted_triple_data.append((data["output_node"], "hasNode", node))
        formatted_triple_data.append(
            (data["output_node"], str(rdflib.RDF.type), str("Recipe"))
        )

        progress += 1
        if progress % 1000 == 0:
            print(
                f"progress: {round(progress/len(recipe_data.keys()), 4)} - {round(time.time()-starttime,3)}seconds"
            )
    ...
    # split into train/val/test splits, with 70/15/15 percent of data in each
    graph_train, graph_test = train_test_split(formatted_graph_data, test_size=0.15)
    graph_train, graph_val = train_test_split(graph_train, test_size=0.17647)

    print(f"writing results, {len(formatted_graph_data)} total graph data ")
    train, test = train_test_split(formatted_triple_data, test_size=0.15)
    train, val = train_test_split(train, test_size=0.17647)
    ...
    print("saving graphs")
    with open((output_dir / "test.txt").resolve(), "w") as f:
        for i in range(len(graph_test) - 1):
            f.write(f"{json.dumps(graph_test[i])}\n")
        f.write(f"{json.dumps(graph_test[-1])}")

    with open((output_dir / "valid.txt").resolve(), "w") as f:
        for i in range(len(graph_val) - 1):
            f.write(f"{json.dumps(graph_val[i])}\n")
        f.write(f"{json.dumps(graph_val[-1])}")

    with open((output_dir / "train.txt").resolve(), "w") as f:
        for i in range(len(graph_train) - 1):
            f.write(f"{json.dumps(graph_train[i])}\n")
        f.write(f"{json.dumps(graph_train[-1])}")
    ...
    with open((output_dir / "trip_test.txt").resolve(), "w") as f:
        for i in range(len(test) - 1):
            if test[i] not in test_overlap:
                f.write(f"{test[i][0]}\t{test[i][1]}\t{test[i][2]}\n")
        f.write(f"{test[-1][0]}\t{test[-1][1]}\t{test[-1][2]}")

    with open((output_dir / "trip_valid.txt").resolve(), "w") as f:
        for i in range(len(val) - 1):
            if val[i] not in val_overlap:
                f.write(f"{val[i][0]}\t{val[i][1]}\t{val[i][2]}\n")
        f.write(f"{val[-1][0]}\t{val[-1][1]}\t{val[-1][2]}")

    with open((output_dir / "trip_train.txt").resolve(), "w") as f:
        for i in range(len(train) - 1):
            f.write(f"{train[i][0]}\t{train[i][1]}\t{train[i][2]}\n")
        f.write(f"{train[-1][0]}\t{train[-1][1]}\t{train[-1][2]}")
...
if __name__ == "__main__":
    ...
    output_dir = (main_dir / "eatpim_triple_data/").resolve()
